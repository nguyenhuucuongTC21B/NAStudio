    ...
    return {
      ...
      async CloudSynthesize(text, voice) {
        ...
        const tick = () => {
          ...
          const [st, pct, msg] = steps[i++];
          Bus.emitLocal("hcstudio:job", { id, state: st, pct, message: msg, elapsedSec: i * 1.4 });
          setTimeout(tick, 650);
        };
        setTimeout(tick, 250);
        return id;
      },
      async CloudPickTextFile() {
        return "Đây là nội dung file mẫu trong chế độ preview trình duyệt.\n\nTrong bản Windows, nút này mở hộp thoại chọn file .txt / .srt / .md thật và nạp nội dung vào ô văn bản.";
      },
      async OpenFolder() {}, 
