...
type Settings struct {
	...
	CloudState string    `json:"cloudState,omitempty"` // PATCH FIX53: snapshot trạng thái chuỗi dịch vụ online
	CloudVoice string    `json:"cloudVoice,omitempty"` // PATCH FIX53: giọng online người dùng chọn
...
func mergeValid(dst *Settings, src Settings) {
	...
	// PATCH FIX53: trạng thái chuỗi online (JSON) + giọng online — nhận
	// nguyên đoạn JSON (chặn độ dài để tránh phình settings).
	if len(src.CloudState) <= 65536 {
		dst.CloudState = src.CloudState
	}
	if len(src.CloudVoice) <= 128 {
		dst.CloudVoice = src.CloudVoice
	}
}
