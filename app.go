...
type App struct {
	...
	playJobID   string
	modelDLRun  bool
	cloudChain  *cloud.Chain // PATCH FIX53: chuỗi cầu nối online có nhớ trạng thái
...
func (a *App) startup(ctx context.Context) {
	...
	// PATCH FIX53: nạp trạng thái chuỗi online đã lưu (dịch vụ nào cạn lượt
	// hôm nay, cái nào thành công gần nhất) — hết việc thử lại oan.
	a.cloudChain = cloud.NewChain(a.settings.CloudState)
	if a.cloudChain.LastGoodID() != "" {
		diagf("startup: chuỗi online nạp xong — dịch vụ thành công gần nhất: %s", a.cloudChain.LastGoodID())
	}
...
func (a *App) SaveSettings(s appstate.Settings) {
	...
	// PATCH FIX53: giọng online (cho phép rỗng = mặc định từng dịch vụ)
	if len(s.CloudVoice) < 128 {
		a.settings.CloudVoice = s.CloudVoice
	}
...
// CloudProviderInfo một dòng trong bảng "Dịch vụ Online" của UI.
...
func (a *App) CloudProviders() []CloudProviderInfo {
	...
	for _, d := range reg {
		st := a.cloudChain.StatusOf(d.ID)
		out = append(out, CloudProviderInfo{
			ID: d.ID, Label: d.Label, Kind: d.Kind, Note: d.Note,
			SkipReason: d.SkipReason, Status: st.Status, Err: st.Err,
			CountToday: st.CountToday, LastOKAt: st.LastOKAt,
			DefaultVoice: d.DefaultVoice, Voices: d.Voices,
		})
	}
	return out
}
...
// thường: PHÁT / xuất WAV·MP3 / danh sách phát dùng lại nguyên hệ FIX52.
func (a *App) CloudSynthesize(text, voiceName string) string {
	jobID := genID("cloud")
	...
	a.mu.Lock()
	if a.cancelSynth != nil {
		a.cancelSynth() // huỷ job cũ (offline hoặc online) nếu còn chạy
	}
	a.cancelSynth = cancel
	a.mu.Unlock()
...
// runCloudSynthesis chạy chuỗi cầu nối trong goroutine; mỗi mốc của chuỗi
// phát thành hcstudio:job để UI dùng lại đúng thanh tiến độ + đồng hồ chờ
// của FIX52 (hết cảnh "bấm xong không biết máy đang làm gì").
func (a *App) runCloudSynthesis(ctx context.Context, jobID, text, voiceName string) {
	defer func() {
		if r := recover(); r != nil {
			diagf("[error] cloud %s: PANIC goroutine online: %v · stack:\n%s", jobID, r, debug.Stack())
			a.pushJob(jobID, "error", "Lỗi nội bộ chế độ online: "+fmt.Sprint(r), 0)
		}
	}()

	a.pushJob(jobID, "splitting", "Chuẩn bị gửi lên dịch vụ online…", 1)
	res, err := a.cloudChain.Synthesize(ctx, nil, cloud.Request{Text: text, VoiceName: voiceName},
		func(ev cloud.Event) {
			a.pushJobElapsed(jobID, "synthesizing", ev.Pct, ev.Message, 0, ev.ElapsedSec, false)
			if ev.Phase != "waiting" { // heartbeat thì khỏi spam log
				diagf("cloud %s: [%s] %s", jobID, ev.Phase, ev.Message)
			}
		})

	// persist trạng thái chuỗi (đếm lượt + cạn lượt theo ngày + last good)
	a.mu.Lock()
	a.settings.CloudState = a.cloudChain.SnapshotJSON()
	_ = a.settings.Save()
	a.mu.Unlock()

	if err != nil {
		if ctx.Err() != nil {
			a.pushJob(jobID, "cancelled", "Đã huỷ", 0)
			return
		}
		diagf("[error] cloud %s: toàn bộ chuỗi thất bại: %v", jobID, err)
		a.pushJob(jobID, "error", truncate(err.Error(), 220), 0)
		a.toast("error", "Chế độ online thất bại", truncate(err.Error(), 260))
		return
	}
	...
	sess := &Session{
		ID:       jobID,
		VoiceID:  res.VoiceUsed,
		Engine:   "cloud:" + res.ProviderID,
		Text:     text,
		Samples:  samples,
		SR:       sr,
	...
	a.putSession(sess)
	diagf("cloud %s: HOÀN TẤT · provider=%s · voice=%q · %.1fs @%dHz · đã thử %d dịch vụ",
		jobID, res.ProviderID, res.VoiceUsed, sess.Duration, sr, res.Tried)
	a.pushJobDone(jobID, sess.Duration)
	a.toast("success", "Chế độ online — hoàn tất",
		fmt.Sprintf("%s · giọng %s · %.1fs audio (đã thử %d dịch vụ)",
			res.ProviderLabel, res.VoiceUsed, sess.Duration, res.Tried))

	// nghe ngay sau khi có kết quả — đúng tinh thần "Nghe ngay" của FIX52
	if perr := a.PlayJob(jobID); perr != nil {
		diagf("[warn] cloud %s: tự phát sau khi nhận audio lỗi: %v", jobID, perr)
	}
}

// CloudPickTextFile mở hộp thoại chọn file văn bản (.txt/.srt/.md) rồi đọc
// nội dung trả về UI — giới hạn 200.000 ký tự như khung nhập chính.
func (a *App) CloudPickTextFile() string {
	path, err := wailsruntime.OpenFileDialog(a.ctx, wailsruntime.OpenDialogOptions{
		Title: "Chọn file văn bản cần chuyển thành giọng nói",
		Filters: []wailsruntime.FileFilter{
			{DisplayName: "Văn bản (.txt · .srt · .md)", Pattern: "*.txt;*.srt;*.md"},
		},
	})
	if err != nil || path == "" {
		return ""
	...
	if err != nil {
		diagf("[error] cloud: đọc file %s lỗi: %v", path, err)
		a.toast("error", "Không đọc được file", truncate(err.Error(), 180))
		return ""
	}
	content := string(raw)
	if r := []rune(content); len(r) > maxTextLen {
		content = string(r[:maxTextLen])
		a.toast("warn", "File rất dài",
			fmt.Sprintf("Chỉ lấy %s ký tự đầu tiên (giới hạn của khung nhập).", "200.000"))
	}
	diagf("cloud: nạp file %s · %d ký tự", path, len([]rune(content)))
	return content
}
